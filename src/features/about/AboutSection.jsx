...
import { GraduationCap, Heart, Lightbulb, Compass, ShieldCheck, Award, CheckCircle2 } from "lucide-react";
...
function AboutAvatar() {
  ...
  return (
    <div className="relative shrink-0">
      <div className="w-11 h-11 rounded-md overflow-hidden border border-[#1C4E50] bg-[#0E2A2B] shadow-subtle grid place-items-center">
        {!hasError ? (
          <img
            src={imgSrc}
            alt="Shiva Kasaudhan"
          ...
          <div className="w-full h-full bg-[#E8B84B] grid place-items-center text-[#0E2A2B] font-bold text-sm">
            SK
          </div>
        )}
      </div>
...
export function AboutSection() {
  return (
    <section id="about" className="relative py-20 sm:py-28 px-4 sm:px-6 lg:px-8 bg-[#0E2A2B] overflow-hidden border-t border-[#1C4E50]">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div>
          ...
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-4">
            {stats.map((stat, i) => (
              <motion.div
                ...
                className="editorial-card p-5 text-center bg-[#133637] border border-[#1C4E50] rounded-md"
              >
                <div className="text-2xl sm:text-3xl font-extrabold text-[#E8B84B] font-mono">
                  <Counter value={stat.value} suffix={stat.suffix} decimals={stat.decimals} />
                </div>
                <div className="mt-1.5 text-[10px] sm:text-xs text-[#A1B5B3] uppercase tracking-wider font-mono font-medium">
                  {stat.label}
                </div>
              </motion.div>
            ))}
          ...
          <div className="mt-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-7 space-y-4">
              {story.map((s, i) => (
                <motion.div
                  ...
                  className="editorial-card p-6 bg-[#133637] border border-[#1C4E50] rounded-md"
                >
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 grid place-items-center rounded-md bg-[#0E2A2B] border border-[#1C4E50] text-[#E8B84B] shrink-0">
                      <s.icon className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="font-bold text-base text-[#F4EFE6]">{s.title}</h3>
                      <p className="mt-1.5 text-xs sm:text-sm text-[#A1B5B3] leading-relaxed font-sans">{s.text}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
            ...
            <motion.div
              ...
              className="lg:col-span-5 editorial-card p-7 space-y-6 bg-[#133637] border border-[#1C4E50] rounded-md"
            >
              <div className="flex items-center gap-2 text-xs font-mono text-[#E8B84B] font-bold">
                <Compass className="w-4 h-4" /> Professional Philosophy
              </div>
              
              <blockquote className="text-sm sm:text-base text-[#F4EFE6] font-serif italic leading-relaxed">
                "Great web products are created when intentional UI design, structured code, and performance serve real user goals."
              </blockquote>

              <div className="pt-4 border-t border-[#1C4E50] flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <AboutAvatar />
                  <div>
                    <div className="font-bold text-sm text-[#F4EFE6]">Shiva Kasaudhan</div>
                    <div className="text-xs text-[#A1B5B3] font-mono">Frontend Developer • MCA Graduate</div>
                  </div>
                </div>
                <div className="font-serif italic text-[#E8B84B] text-lg sm:text-xl font-bold shrink-0">
                  Shiva K.
                </div>
              </div>
            </motion.div>
        ...
        <div>
          <motion.div
            ...
            className="mb-8"
          >
            <div className="inline-flex items-center gap-2 text-xs font-mono text-[#E8B84B] font-bold uppercase tracking-widest mb-2">
              <Award className="w-3.5 h-3.5" /> Key Honors & Recognitions
            </div>
            <h3 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-[#F4EFE6]">
              Verified <span className="text-[#E8B84B] font-serif italic font-normal">Academic Achievements</span>
            </h3>
          </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {achievements.map((ach, i) => (
              <motion.div
                ...
                className="editorial-card p-5 space-y-2 flex flex-col justify-between bg-[#133637] border border-[#1C4E50] rounded-md"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full bg-[#0E2A2B] text-[#E8B84B] border border-[#1C4E50]">
                      {ach.tag}
                    </span>
                    <CheckCircle2 className="w-4 h-4 text-[#E8B84B]" />
                  </div>
                  <h4 className="font-bold text-base text-[#F4EFE6] leading-snug">{ach.title}</h4>
                  <p className="text-xs text-[#A1B5B3] leading-relaxed font-sans">{ach.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
